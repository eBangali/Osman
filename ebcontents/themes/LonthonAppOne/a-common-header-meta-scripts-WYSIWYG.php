...
var showingSourceCodeHow;
var isInEditModeHow;
//
function enableEditModeHow()
{
richTextFieldHowToDo.document.designMode = 'On';
showingSourceCodeHow = false;
isInEditModeHow = true;
}
//
function updateIframeHow()
{
var viewHtmlCode = document.getElementsByTagName("richTextFieldHowToDo").innerHTML;
document.getElementsById("#HowToDo").innerHTML = viewHtmlCode;
alert("How to Clipping path you should do?")
}
//
function execCmd(command)
{
richTextFieldHowToDo.document.execCommand(command, false, null);
}
//
function execCommandWithArg(command, arg)
{
richTextFieldHowToDo.document.execCommand(command, false, arg);
}
//
function toggleSource()
{
if(showingSourceCodeHow)
{
richTextFieldHowToDo.document.getElementsByTagName('body')[0].innerHTML = richTextFieldHowToDo.document.getElementsByTagName('body')[0].textContent;
showingSourceCodeHow = false;
}
else
{
richTextFieldHowToDo.document.getElementsByTagName('body')[0].textContent = richTextFieldHowToDo.document.getElementsByTagName('body')[0].innerHTML;
showingSourceCodeHow = true;
}
}
</script>
<style>
.classHowtodo{ display:none; }
.ebEditor label{
margin: 3px;
padding: 3px;
}
.ebEditor label:hover{
	background: #CCC;
}
</style>
</head>
<body onLoad="enableEditModeHow();">