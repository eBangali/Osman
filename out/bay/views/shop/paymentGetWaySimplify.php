...
<button data-sc-key='<?php $merchant = new ebapps\bay\ebcart(); $merchant -> paymentOptionSimplifyExits(); if($merchant -> eBData >= 1) { foreach($merchant -> eBData as $val){ extract($val); echo $payment_gateways_public_key; }} ?>'
data-name='<?php echo domain; ?>'
data-description='eCommerce'
data-reference='<?php echo $_SESSION['order_tracking_unique_id']; ?>'
data-currency='<?php echo primaryCurrency ?>'
data-amount='<?php echo intval(number_format($_SESSION['total_payment'],2,'','')); ?>'
data-color='#12B830'
data-redirect-url='<?php echo outBayLink; ?>/views/shop/thankyou-from-simplify.php'
>Pay Now
</button>
</fieldset>
</form>
