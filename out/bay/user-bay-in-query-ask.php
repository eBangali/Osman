...
elseif (! preg_match("/^([a-zA-Z0-9\,\.\?\ ]{3,100})/",$bay_support_requirements))
{
$bay_support_requirements_error = "<b class='text-warning'>Use A-Za-z0-9.,? mini 3 max 100.</b>";
$error =1;
}
else
{
...
$user->bay_submit_a_query_buyer($bay_product_id_in_buyer_support, $bay_order_tracking_id, $bay_username_seller, $bay_support_requirements);
}
}
?>
<?php
$obj = new ebapps\bay\ebcart();
$obj->bay_read_query_to_submit_another_one();
if($obj->eBData > 0)
{
...
$queryMe .="<input type='hidden' name='bay_product_id_in_buyer_support' value='$bay_product_id_in_buyer_support' /><input type='hidden' name='bay_order_tracking_id' value='$bay_order_tracking_id' /><input type='hidden' name='bay_username_seller' value='$bay_username_seller' />"; 
