...
<div class='well'>
<h2 title='Add category D'>Add category D</h2>
</div>
...
({
type: "POST",
url: "bay_merchant_add_items_select_c_from_b.php",
data: "pic_name="+ pic_name,
success: function(option)
{
$("#category_c").html("<option value=''>Please Select</option>"+option);
}
});
}
else
{
$("#category_c").html("<option value=''>Please Select</option>");
}
return false;
});
});
...
if($error ==0){
extract($_REQUEST);
$user->submit_category_d($category_a, $category_b, $category_c, $category_d);
}
//
}
?>
...
Select Category C: <?php echo $category_c_error;  ?>
<select class='form-control' id='category_c' name='category_c'><option>Please Select</option></select>
Category D: <?php echo $category_d_error;  ?>
<input class='form-control' type='text' name='category_d' placeholder="Men-s-T-shirts will be shown as Men's T-shirts" required autofocus />
<div class='buttons-set'>
<button type='submit' name='category_d_submit' title='Submit' class='button submit'> <span> Submit </span> </button>
</div>
</fieldset>
</form>
</div>
...
<tbody>
<?php include_once (ebbay.'/ebcart.php'); ?>
<?php $obj= new ebapps\bay\ebcart(); $obj -> select_category_d_to_show(); ?>
<?php if($obj->eBData){ foreach($obj->eBData as $val): extract($val); ?>
<?php 
$zoneDhl = "<tr>";					 
$zoneDhl .= "<td>".$obj->visulString($bay_category_a_in_bay_category_d)."</td>";
$zoneDhl .= "<td>".$obj->visulString($bay_category_b_in_bay_category_d)."</td>";
$zoneDhl .= "<td>".$obj->visulString($bay_category_c_in_bay_category_d)."</td>";
$zoneDhl .= "<td>".$obj->visulString($bay_category_d)."</td>";
$zoneDhl .= "<td><form action='bay-edit-category-d.php' method='get'><input type='hidden' name='category_d_id' value='$bay_category_d_id' /><button type='submit' name='edit_category_d' value='Edit' class='button submit' alt='Edit'><b>Edit</b></button></form></td>";
$zoneDhl .= "</tr>";
echo $zoneDhl;
endforeach;
}
