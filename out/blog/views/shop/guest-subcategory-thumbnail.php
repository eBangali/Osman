...
//
$subCat = new ebapps\blog\blog();
$subCat -> contents_thurmnail_subcategory_gpost($category,$subcategory);
if($subCat->eBData)
{
...
$newSearch .="<h2>".$subCat->visulString($subcategory)."</h2>";
$newSearch .="</div>";
$newSearch .="</li>";
$newSearch .="</ul>";
$newSearch .="</div>";
...
foreach($subCat->eBData as $vaLsubCat): extract($vaLsubCat);
...
$newSearch .="<div class='item-title'><h3>".strtoupper($subCat->visulString($contents_sub_category))."</h3></div>";
...
$newSearch .=outContentsLink."/contents/solve/$contents_id/".$subCat->seoUrl($contents_og_image_title)."/";
